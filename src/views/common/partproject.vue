...
<script>
	// import {
	// 	getBridgeTree, getTree
	// } from "@/api/tree";
	import { getTree as getBridgeTree } from '@/api/project.js'
	...
	export default {
		...
		methods: {
			...
			getTreeData(){
				getBridgeTree({type: this.currentModule, projectId: this.$store.getters.currentBiaoDuan.id}).then((res) => {
					const arr = [];
					arr.push(res);
					this.treeInfo = arr;
				});
