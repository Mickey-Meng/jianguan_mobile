		...
		<van-list @load="query" :style="{ height: scrollerHeight }" v-model="loading" :finished="finished"
			...
			<van-swipe-cell class="list" v-for="(item, index) in tableData" :key="index"
				...
				<div class="item" style="" @click="viewDetail(item)">
					...
					<div class="block">
						<div class="block-name">首件工程名称：</div>
						<div class="block-value">{{ item.firstProjectName }}</div>
					</div>
					<div class="block">
						<div class="block-name">具体分项：</div>
						<div class="block-value">{{ item.subProjectDetail }}</div>
					</div>
					<div class="block">
						<div class="block-name">实施日期：</div>
						<div class="block-value">{{ item.buildDate }}</div>
					</div>
					<div class="block">
						<div class="block-name">首件工程通过情况：</div>
						<div class="block-value">{{ item.firstPassExplain }}</div>
					</div>
					<div class="status">
						{{ item.statusStr }}
					</div>
