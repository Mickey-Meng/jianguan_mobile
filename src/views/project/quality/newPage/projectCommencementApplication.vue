			...
			<van-swipe-cell class="list" v-for="(item, index) in tableData" :key="index"
				...
				<div class="item" style="" @click="viewDetail(item)">
					...
					<div class="block">
						...
						<div class="block-value">{{ item.buildSectionNames }}</div>
					</div>
					<!-- <div class="block">
							<div class="block-name">合同规定工期定工期止：</div>
							<div class="block-value">{{ item.contractEndDate }}</div>
						</div>
						<div class="block">
							<div class="block-name">合同规定工期起：</div>
							<div class="block-value">{{ item.contractOpenDate }}</div>
						</div> -->
					<div class="block">
						<div class="block-name">申请开工日期：</div>
						<div class="block-value">{{ item.openDate }}</div>
					</div>
					<div class="block">
						<div class="block-name">计划完工日期：</div>
						<div class="block-value">{{ item.endDate }}</div>
					</div>
					<div class="status">
						{{ item.statusStr }}
					</div>
		...
		query() {
			...
			api.getProjectOpenList(this.queryData).then((res) => {
				res.data.list.forEach(element => {
					if (element.buildSectionNames && element.buildSectionNames.length > 0) {
						element.buildSectionNames = element.buildSectionNames.join('、')
					}
