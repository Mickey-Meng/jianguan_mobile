		...
		<van-list @load="query" :style="{ height: scrollerHeight }" v-model="loading" :finished="finished" finished-text="没有更多了">
				<van-swipe-cell class="list" v-for="(item, index) in tableData" :key="index" :class="'item ' + (index === tableData.length - 1 ? 'last' : '')">
					<div class="item" style="" @click="viewDetail(item)">
						<!-- <div class="block">
							<span class="block-name">编制单位</span>
							<span class="block-value">{{ item.pro }}</span>
						</div> -->
						<div class="block">
							<div class="block-name">编制人：</div>
							<div class="block-value">{{ item.compileUser }}</div>
						</div>
						<div class="block">
							<div class="block-name">编制时间：</div>
							<div class="block-value">{{ item.compileDate }}</div>
						</div>
						<div class="block">
							<div class="block-name">制度内容：</div>
							<div class="block-value">{{ item.regimeContent }}</div>
						</div>
						<div class="block">
							<div class="block-name">更新时间：</div>
							<div class="block-value">{{ item.updateTime }}</div>
						</div>
								...
								<div class="form-block">
									...
									<div class="block-line">
										<div class="block-item">
											...
											<div class="block-item-value">{{formData.compileUser}}</div>
										</div>
									</div>
									<div class="block-line">
										<div class="block-item">
									...
									<div class="block-line">
										<div class="block-item">
											...
											<div class="block-item-value">
												<el-input type="textarea" style="min-width: 150px" :rows="15" placeholder="请输入" v-model="formData.regimeContent" :disabled="previewMode"></el-input>
											</div>
										</div>
									</div>
								</div>
			...
			viewDetail(row) {
				...
				api.getManageRegime(row.id).then((res) => {
				  	if (res && res.data) {
						...
						getUserInfo({
							id: res.data.compileUser,
							type: 'app',
							systemName: 'project', 
							noToken: true
						}).then(res => {
							that.formData.userName = res.userInfo.NAME;
							that.formData.compileDeptName = res.userInfo.GROUPNAME;
							that.$forceUpdate();
						});
					}
				});
