 ...
 * @LastEditors: zhouhai
 * @LastEditTime: 2022-06-21 14:10:57
-->
<template>
	<el-container class="container-box">
		<van-list @load="query" :style="{ height: scrollerHeight }" v-model="loading" :finished="finished" finished-text="没有更多了">
				<van-swipe-cell class="list" v-for="(item, index) in tableData" :key="index" :class="'item ' + (index === tableData.length - 1 ? 'last' : '')">
					<div class="item" style="" @click="viewDetail(item)">
						...
						<div class="block">
							<div class="block-name">旁站项目：</div>
							<div class="block-value">{{ item.sideProjectName }}</div>
						</div>
						<div class="block">
							<div class="block-name">旁站人：</div>
							<div class="block-value">{{ item.createName }}</div>
						</div>
						<div class="block">
							<div class="block-name">工程部位：</div>
							<div class="block-value">{{ item.projectPartStr }}</div>
						</div>
						<div class="block">
							<div class="block-name">旁站时间：</div>
							<div class="block-value">{{ item.sideDate }}</div>
						</div>
						<div class="status">
							{{ item.statusStr }}
						</div>
		...
		<!-- <el-button round icon="plus" style="position:absolute;right:10px;bottom:10px;"></el-button> -->
		<el-button icon="el-icon-plus" circle style="position:absolute;right:10px;bottom:10px;" @click="addNew"></el-button>
	</el-container>
</template>
