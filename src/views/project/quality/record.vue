    ...
    >
      ...
      >
        ...
        <div class="content">
          ...
          <span class="value">{{ item.qualityfirstname }}</span>
        </div>
        <div class="content">
          <span class="name">检查小项：</span>
          <span class="value">{{ item.qualitysecondname }}</span>
        </div>
        <div class="content">
...
export default {
  ...
  methods: {
    ...
    onLoad() {
      ...
      getAllStatusQualityEvent(params).then(data => {
        that.list = data.sort(function(a, b) {
          return Date.parse(b['uploadtime'].replace(/-/g, '/')) - Date.parse(a['uploadtime'].replace(/-/g, '/'))
        })
