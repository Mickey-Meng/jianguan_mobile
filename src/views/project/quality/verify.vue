...
<script>
import { getNotDoneQualityEvent, doNotDoneQualityEvent } from '@/api/project.js'
...
export default {
  ...
  methods: {
    onLoad() {
      let that = this
      getNotDoneQualityEvent({'projectId':this.currentBiaoDuan.id}).then(data => {
        that.list = data || []
    ...
    submit(action, done) {
      if (action === 'cancel') {
        ...
        doNotDoneQualityEvent(this.delayItem).then(data => {
          this.$toast('处理成功')
