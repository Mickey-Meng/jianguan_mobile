...
<script>
import { getSafeEvent, submitDelaySafeEvent } from '@/api/project.js'
export default {
  components: {},
  data() {
    return {
  ...
  methods: {
    onLoad() {
      let that = this
      getSafeEvent({projectId: this.$store.state.currentBiaoDuan.id}).then(data => {
        let temps = []
    ...
    delaySubmit(action, done) {
      if (action === 'cancel') {
        ...
        submitDelaySafeEvent(this.delayItem).then(data => {
          this.$toast('处理成功')
