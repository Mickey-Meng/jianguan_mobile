		...
		<van-list @load="query" :style="{ height: scrollerHeight }" v-model="loading" :finished="finished"
			...
			<van-swipe-cell class="list" v-for="(item, index) in tableData" :key="index"
				...
				<div class="item" style="" @click="viewDetail(item)">
					...
					<div class="block">
						<div class="block-name">拟劳务合作工程名称：</div>
						<div class="block-value">{{ item.laborContractProjectName }}</div>
					</div>
					<div class="block">
						<div class="block-name">备案日期：</div>
						<div class="block-value">{{ item.startDate }}</div>
					</div>
					<div class="block">
						<div class="block-name">承包人：</div>
						<div class="block-value">{{ item.contractUser }}</div>
					</div>
					<div class="status">
						{{ item.statusStr }}
					</div>
		...
		query() {
			...
			api.getContractLaborList(this.queryData).then((res) => {
				res.data.list.forEach(element => {
					element.laborContractProjectName = element.laborContractProjectName.join("、")
				})
