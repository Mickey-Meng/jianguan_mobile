			...
			<van-list @load="query" :style="{ height: scrollerHeight }" v-model="loading" :finished="finished" finished-text="没有更多了">
				<van-swipe-cell class="list" v-for="(item, index) in tableData" :key="index" :class="'item ' + (index === tableData.length - 1 ? 'last' : '')">
					<div class="item" style="" @click="viewDetail(item)">
						...
						<div class="block">
							<div class="block-name">账单编号：</div>
							<div class="block-value">{{ item.billCode }}</div>
						</div>
						<div class="block">
							<div class="block-name">付款单位：</div>
							<div class="block-value">{{ item.payUnit }}</div>
						</div>
						<div class="block">
							<div class="block-name">收款单位：</div>
							<div class="block-value">{{ item.gatherUnit }}</div>
						</div>
						<div class="block">
							<div class="block-name">支付金额(元)：</div>
							<div class="block-value">{{ item.payAmount }}</div>
						</div>
						<div class="status">
							{{ item.statusStr }}
						</div>
